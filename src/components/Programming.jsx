    ...
    <div className="pb-20">
       <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-lg shadow mb-4 border border-blue-100 dark:border-slate-700 gap-3">
           ...
           <div className="flex gap-2 w-full md:w-auto">
               <button onClick={()=>setShowModal(true)} className="flex-1 md:flex-none bg-blue-600 text-white text-xs px-3 py-2 rounded font-bold shadow hover:bg-blue-700 whitespace-nowrap">+ Agendar Nuevo</button>
               <button onClick={exportCSV} className="flex-1 md:flex-none bg-green-600 text-white text-xs px-3 py-2 rounded font-bold shadow hover:bg-green-700 flex items-center justify-center gap-1 whitespace-nowrap"><Download size={14}/> CSV</button>
           </div>
       </div>
       ...
       <div className="space-y-6">
           {sortedDates.map(dateKey => {
               ...
               const dateLabel = dateObj.toLocaleDateString('es-MX', { weekday: 'long', day: 'numeric', month: 'long' }).toUpperCase();
           ...
           {sortedDates.length === 0 && <div className="text-center p-10 text-slate-400">No hay cirugías programadas próximamente.</div>}
       </div>
